...
class PlasmaPsiQRHPIDAvancado:
    ...
    def __init__(self, N=50):
        ...
        self.K_p_base, self.K_i_base, self.K_d_base = 30, 12, 10  # Base values
    ...
    def gain_scheduling_pid(self, sync_current):
        ...
        if sync_current < 0.4:
            # Moderate response for low synchronization
            K_p, K_i, K_d = 35, 10, 8
            modo = "MODERATE"
        elif sync_current < 0.6:
    ...
    def detectar_oscilacoes(self):
        ...
        if len(self.historico_sync) >= 20:
            # FFT of last 20 sync points
            fft_sync = np.fft.fft(self.historico_sync[-20:])
            magnitudes = np.abs(fft_sync[1:10])  # Ignore DC and high frequencies

            # Detect persistent oscillations
            if np.max(magnitudes) > 5:  # Threshold for strong oscillation
    ...
    def sistema_alarme(self, sync_current, t):
        ...
        if sync_current < 0.3 and t > 5 and not self.alerta_emitido:  # Lower threshold
            print("🚨 CRITICAL ALERT: Synchronization below 0.3! Activating emergency measures!")
            self.K_acustico *= 1.1  # Smaller increase
            self.K_lider_base *= 1.05  # Smaller increase
            self.alerta_emitido = True
            return True

        # Reset alert if recovered
        if sync_current > 0.5 and self.alerta_emitido:  # Lower recovery threshold
    ...
    def ativar_super_cruzeiro(self, sync_current):
        ...
        if self.regime_cruzeiro and sync_current > 0.95 and not self.super_cruzeiro:
            ...
            self.omega_plasma = 7.5  # Even smoother
            self.K_lider *= 0.5      # Minimal control
    ...
    def controle_pid_avancado(self, sync_current, t):
        ...
        # TEMPORARY BOOST
        if sync_current > 0.7 and not self.regime_cruzeiro:
            ganho_pid = min(85, ganho_pid + 10)  # Smaller boost
            modo_pid = "BOOST"
    ...
    def forca_acustica_avancada(self, t, angulo_direcao=np.pi/4):
        ...
        for cx, cy in self.transdutores:
            ...
            if self.super_cruzeiro:
                termo_ressonante = np.sin(0.01 * t)  # Very smooth in super-cruise
                amplitude = 1.0
...
# RUN ADVANCED SIMULATION FOR 90 SECONDS
print("🚀 STARTING ΨQRH SIMULATION WITH ADVANCED CONTROL (90 SECONDS)...")
plasma_avancado = PlasmaPsiQRHPIDAvancado(N=50)

print("Running simulation with all advanced features (900 steps = 90 seconds)...")
resultados = []
eventos_especiais = []

for i in range(900):  # 900 steps * 0.1 = 90 seconds
    t = i * 0.1
    ...
    if modo_pid in ["BOOST", "DAMPED", "AGGRESSIVE"] and i % 50 == 0:  # Less frequent logging
        eventos_especiais.append(f"t={t:.1f}s: {modo_pid}")

    # Progressive feedback every 50 steps
    if i % 50 == 0 or modo_pid in ["BOOST", "DAMPED"]:
...
# ADVANCED REPORT
print("\n" + "="*80)
print("FINAL REPORT - ADVANCED CONTROL (90 SECONDS)")
print("="*80)
...
for evento in eventos_especiais[-10:]:  # Last 10 events
    print(f"   • {evento}")
...
if super_final:
    ...
    print(f"   • Achieved cruise mode in 90 seconds")
else:
    print(f"\n📈 IN PROGRESS: Sync = {sync_final:.3f}")
    print(f"   • System still climbing after 90 seconds")
